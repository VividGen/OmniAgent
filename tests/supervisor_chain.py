...
@pytest.mark.parametrize("query,expected_role", [
    ...
    ("Can you provide a detailed analysis of the Uniswap project, including its recent developments and market position?", "research_analyst_agent"),
    ("What's the weather like today in New York?", "fallback_agent"),
    ("I want to invest in a promising DeFi project. Can you first give me an overview of the current DeFi market trends, then recommend a project, and finally show me how to acquire some tokens of that project?", "market_analysis_agent"),
...
def test_next_role(setup_llm, query, expected_role):
