...
class TestNextRole(unittest.TestCase):
    ...
    def test_multi_step_query(self):
        query = "I want to invest in a promising DeFi project. Can you first give me an overview of the current DeFi market trends, then recommend a project, and finally show me how to acquire some tokens of that project?"
        expected_role = "market_analysis_agent"
        result = next_role(query)
        self.assertEqual(result, expected_role,
                         f"Expected {expected_role}, but got {result} for query: {query}")
