...
from omniagent.agent.ctx_var import chat_req_ctx, resp_msg_id
...
from omniagent.dto.cb_content import CbContent, CbContentType
...
async def arun_agent(req: ChatReq, stream_cb: StreamCallbackHandler, resp_msg_id0: str) -> Any:
    ...
    return await agent.ainvoke(
        ...
        config={
            "metadata": {"agentName": "omniagent-backend", "userId": req.user_id},
            "callbacks": [stream_cb],
        },
    )
...
async def gen_agent_resp(cb_content: CbContent, resp_msg_id0: str) -> str:
...
async def need_create_session(req: ChatReq) -> bool:
