...
contract Deploy is Deployer {
    ...
    function deployProxies() public {
        deployProxy("OmniAgentExecutorManager");
    }
    ...
    function deployManager() public broadcast returns (address addr_) {
        OmniAgentExecutorManager manager = new OmniAgentExecutorManager();

        // check states
        require(!manager.hasRole(DEFAULT_ADMIN_ROLE, cfg.manager()), "admin role error");

        save("OmniAgentExecutorManager", address(manager));
        console.log("OmniAgentExecutorManager deployed at %s", address(manager));
        addr_ = address(manager);
    }

    function initializeManager() public broadcast {
        OmniAgentExecutorManager managerProxy = OmniAgentExecutorManager(
            mustGetAddress("OmniAgentExecutorManagerProxy")
        );
