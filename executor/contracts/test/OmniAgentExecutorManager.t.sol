...
import {OmniAgentExecutorManager} from "../src/OmniAgentExecutorManager.sol";
import {OmniAgentExecutor} from "../src/OmniAgentExecutor.sol";
import {TransparentUpgradeableProxy} from "../src/upgradeability/TransparentUpgradeableProxy.sol";
import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";

contract OmniAgentExecutorManagerTest is CommonTest {
    // events
    ...
    function testInitialize() public {
        OmniAgentExecutorManager managerImpl = new OmniAgentExecutorManager();
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(managerImpl),
            proxyAdmin,
            ""
        );
        OmniAgentExecutorManager(payable(address(proxy))).initialize(admin);
    ...
    function testInitializeFail() public {
        OmniAgentExecutorManager newImpl = new OmniAgentExecutorManager();
        // upgrade
        vm.prank(proxyAdmin);
    ...
    function testCreateExecutorFailed(uint256 userId) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                OmniAgentExecutorManager__NoRoleToCreateExecutor.selector,
                address(alice)
            )
        );
        vm.prank(alice);
        ...
        // create executor
        vm.expectRevert(
            abi.encodeWithSelector(OmniAgentExecutorManager__InValidUserId.selector, userId)
        );
        vm.prank(admin);
        _manager.createExecutor(userId);
